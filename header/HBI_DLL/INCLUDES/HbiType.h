...
#define QUEUE_MAX_SIZE        16    // Maximum buffer value
#define MODE_LIST_FIREWARE_TOTAL       10
#define MODE_LIST_TOTAL       100
#define MODE_ITEM_NUMBER      11
#define MODE_LIST_ITEM_NUMBER 111
#define MAX_IMAGE_BINNING     6  // ADD BY MHYANG 20250220
// Flat panel detector's status
// 平板状态
// Static fpd status
// 静态平板状态
#define STATIC_FPD_STATUS_BUSY         1  // fpd busy status
#define STATIC_FPD_STATUS_READY        2  // fpd ready status
#define STATIC_FPD_STATUS_AED          3  // AED mode,avild image data
#define STATIC_FPD_STATUS_PREPARE      4  // Software mode,prepare command feedback command
// Dynamic fpd status,Includes wireless panel
// 动态平板状态，包括无线平板探测器
...
#define DETECTOR_TYPE_NUMBER 0x26
...
static const FpdPixelMatrixTable HB_FPD_SIZE[] = {	
	...
	{34, 2432, 3040, 14440, 1805, 100},/*,"2530CMO-100um"*/
	{35, 2400, 3032, 14213, 1771, 100},/*,"2530CMO-100um"*/
	{36, 2560, 2048, 10240, 1280, 99 },/*,"2520z-99um"*/
	{37, 7680, 3072, 46080, 5760, 140},/*,"43108a-140um"*/
	{38, 6144, 3072, 36864, 4608, 140} /*,"4386a-140um"*/
};
...
typedef enum
{
	...
	OFFSET_TEMPLATE_RAW = 0x0a,
	GAIN_TEMPLATE_RAWA = 0x0b,
	GAIN_TEMPLATE_RAWB = 0x0c,
	DEFECT_TEMPLATE_RAW = 0x0d,
	DEFECT_TEMPLATE_MAP = 0x0e,
	DEFECT_TEMPLATE_TXT = 0x0f
}emTEMPLATE_TYPE;
...
typedef enum
{
	WIRELESS_WIRED_MODE = 0x00,
	WIRELESS_AP_MODE    = 0x01,
	WIRELESS_STA_MODE   = 0x02
}emNETWORK_MODE;
typedef enum
{
	OPEN_IMAGE_SERVER   = 0x00,
	CLOSE_IMAGE_SERVER  = 0x01,
	OPEN_BRAM_SERVER    = 0x02,
	CLOSE_BRAM_SERVER   = 0x03,
	OPEN_MCU_SERVER     = 0x04,
	CLOSE_MCU_SERVER    = 0x05,
	OPEN_EVENT_SERVER   = 0x06,
	CLOSE_EVENT_SERVER  = 0x07,
	OPEN_ALL_SERVER     = 0x08,
	CLOSE_ALL_SERVER    = 0x09,
	WRITE_GPIO          = 0x0a,
	READ_I2C            = 0x0b,
	READ_MCU            = 0x0c,
	QUERY_SERVER_STATUS = 0x0d,
	IMAGE_END_RESPONSE  = 0x0e,
	RESET_FRAME_ID      = 0x0f,
	WRITE_CONFIG_FILE   = 0x10,
	READ_CONFIG_FILE    = 0x11,
	ENABLE_SAVE_IMAGE   = 0x12,
	OPENCV_TEST         = 0x13
}emExtendRequestComm;
...
enum eRequestCommType
{
	...
	EDL_COMMON_TYPE_ROM_TO_FACTORY        = 0X19,// add by MH.YANG 18/2/2025
	...
	EDL_COMMON_SWITCH_NETWORK             = 0x60,//
	EDL_COMMON_EXTEND_SYSTEM_CMD          = 0x61,//
...
enum eCallbackEventCommType
{
	...
	ECALLBACK_TYPE_EMPTY_NODE          = 0X16, // illegal package number
	...
	ECALLBACK_TEMPERATURE_BATTERY_INFO = 0X21, // Firmware temperature BY Ling 20240924 
	ECALLBACK_DEBUG_PRINT_TIME         = 0X22, //BY MHYANG 20250224
	...
	ECALLBACK_TYPE_FRAME_MISS_MSG      = 0X61, // Frame loss information
	ECALLBACK_TYPE_FRAME_MISS_NUM      = 0X62, // Frame loss information
...
// Notice: 
typedef struct FrameCountingSt
{
	unsigned int uBeginFrameId;
	unsigned int uEndFrameId;
	unsigned int uCurDllFrameId;
	unsigned int uCurDllEventId;
}FRAME_COUNTING;

// Notice:
typedef union short_value
{
	struct
	{
		unsigned int  b00 : 1; // Undefined
		unsigned int  b01 : 1; // Undefined
		unsigned int  b02 : 1; // Undefined
		unsigned int  b03 : 1; // Undefined
		unsigned int  b04 : 1; // Fully Discharged
		unsigned int  b05 : 1; // Fully Charged
		unsigned int  b06 : 1; // Discharging or Relax
		unsigned int  b07 : 1; // Initialization
		unsigned int  b08 : 1; // Remaining Time Alarm
		unsigned int  b09 : 1; // Remaining Capacity Alarm
		unsigned int  b10 : 1; // Undefined
		unsigned int  b11 : 1; // Terminate Discharge Alarm
		unsigned int  b12 : 1; // Overtemperature Alarm
		unsigned int  b13 : 1; // Undefined
		unsigned int  b14 : 1; // Terminate Charge Alarm
		unsigned int  b15 : 1; // Overcharged Alarm
	}BITE_VALUE;
...
typedef struct status_info_st
{
	status_info_st()
	{
		...
		uzNoBattery          = 0;
		usBatteryStatus      = 0;
		uBatteryTemperature  = 0;
		usBatteryValtage     = 0;
		uBatteryElectrical   = 0;
		uzBatteryPercentageH = 0;
		uzBatteryPercentageL = 0;
	}
	short          uCoreTemperature;         // Core Temperature
	short          uInnerTemperature;        // Inner Temperature
	unsigned char  uzFpdRunStatus;           // Run Status
	unsigned char  uzNoBattery;              // 0-No battery，1-The battery exists
    unsigned short usBatteryStatus;          // Battery status，refer to ‘UNION_SHORT_VALUE’
	short          uBatteryTemperature;      // Battery temperature
	unsigned short usBatteryValtage;         // Battery HVG
	short          uBatteryElectrical;       // Battery Electrical
	unsigned char  uzBatteryPercentageH;     // High level of battery power data
	unsigned char  uzBatteryPercentageL;     // Low level of battery power data
}STATUS_INFO_ST;
...
typedef struct SysManuInfo_st 
{
	...
	// Register_45
	unsigned char		m_abyEnableFrameLossDetect;         //01    //Enable frame loss detection
	// Register_46
	unsigned char		m_abySyncBatteryInfo;               //01    //Synchronize battery infomation
	// Register_47~Register_50
	unsigned char		m_abySysManuInfoReserved[4];        //04    //Reserved Size
}SysManuInfo;
...
typedef struct SysCfgInfo_st 
{
	...
	// Register_106
	unsigned char		m_abyHvgWindowEnable;               //01	//High voltage synchronous exposure window
	// Register_107
	unsigned char		m_abyBinningDivisor;                //01	//Binning Divisor
	// Register_108
	unsigned char		m_abyQuickClearMode;                //01	//Quick clear mode
	// Register_109
	unsigned char		m_abyQuickClearNumber;              //01	//Quick clear number
	// Register_108~Register_120
	unsigned char		m_abySysCfgInfoReserved[11];        //11	//Reserved Data
...
typedef struct UserAcqModeCfg_st
{
	...
	FPD_MODE_DATA       m_stModeList[MODE_LIST_TOTAL] = {0};//220 //Current Mode List Array
}UserModeCfg;

// this struct for copying the right size of firmware parm. 
typedef struct FirmWareUserModeCfg
{
	unsigned char	m_byHead[4];
	unsigned char	m_byCurModeId;
	FPD_MODE_DATA	m_stModeList[MODE_LIST_FIREWARE_TOTAL];
};
...
// this struct for copying the right size of firmware parm. size = 1024 byte
typedef struct FirmWareRegCfgInfo
{
	// System base Information：100,Register_1~Register_100
	SysBaseInfo m_SysBaseInfo;
	// System Manufacture Information：50,Register_101~Register_150
	...
	ADICOFCfg m_ADICOFCfg;
	// User Mode Parameter Configuration：225,Register_670~Register_894
	FirmWareUserModeCfg m_UserModeCfg;
	///////////////////////
	// Reserved Fields:130,,Register_895~Register_1024
	unsigned char m_byReservedFields[130];
};
