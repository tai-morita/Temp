...
bool CapturerByHBIDlg() {
    ...
    */
    ...
    if (!hbiDeviceCtrl.SetCaptureParams(captureConfig)) {
        LOG_INPROGRESSF("lKkL| Failed to set CaptureConfig for Product Code: %s", kstrProductCode.c_str());
        return false;
    }

    // 撮影パラメータをデバイスに設定した後、確認のためにログに出力する。
    if (!hbiDeviceCtrl.PrintCaptureParams()) {
        LOG_INPROGRESSF("IaqJ| Failed to print CaptureConfig for Product Code: %s", kstrProductCode.c_str());
        return false;
    }

    // 画像プロパティを更新する。
    if (!hbiDeviceCtrl.UpdateImageProperties()) {
        LOG_INPROGRESSF("mPM3| Failed to update image properties for Product Code: %s", kstrProductCode.c_str());
        return false;
    }
    ...
    if (!SaveImage(ka4duiImage, kiImageWidth, kiImageHeight, kwstrSaveFilePath)) {
		LOG_INPROGRESSF("Failed to save image");
		return false;
    }
...
int main()
{
    ...
    if (hModule != nullptr)
    {
        ...
        } else
        {
            ...
            if (bResult) {
                LOG_INPROGRESSF("CapturerByHBIDlg() completed successfully.");
            } else {
                LOG_INPROGRESSF("CapturerByHBIDlg() failed.");
            }
        }
    } else
    {
